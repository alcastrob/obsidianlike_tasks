...
- **Crear o editar una tarea** mediante un diálogo de una sola pantalla, estilo el modal "Create or edit Task" de Obsidian Tasks: descripción (con sugeridor de `[[wikilinks]]` al escribir `[[`, buscando entre todas las notas del vault — y, al escribir `#` tras el nombre de una nota, cambia a sugerir los encabezados de esa nota concreta, para enlazar directamente a una sección con `[[Nota#Encabezado]]`), prioridad, fecha límite/programada/inicio, recurrencia con vista previa en vivo, estado (desplegable), fechas de creación/hecho/cancelado, y dependencias "Before this"/"After this" con búsqueda entre todas las tareas del vault — acepta fechas relativas como "today" o "next monday". El diálogo conserva lo escrito aunque cambies a otra pestaña y vuelvas, y no se abre duplicado si "Editar tarea" se dispara dos veces seguidas para la misma tarea (trae al frente el que ya está abierto).
...
- **Bloques de consulta** ` ```tasks ```, con una sintaxis muy cercana a la del plugin original:
  - Filtros: `not done`, `done`, `status.type is [not] <TIPO>`, `status.name includes/does not include <texto>` (para seleccionar por nombre de status, p. ej. `Delegated`/`Waiting`, que no tienen un `.type` propio distinto de TODO/ON_HOLD), `<due|scheduled|start|done|created|cancelled> before/after/on <fecha>`, `no/has <campo> date`, `happens before/after/on <fecha>` / `has/no happens date` (due, scheduled o start a la vez), `has/no depends on`, `has/no id`, `priority is [above|below] <nivel>`, `path`/`description`/`tags`/`heading includes`, `description regex matches /patrón/flags`, `is [not] recurring`.
...
- `renderTasksQuery(queryText)` — ejecuta una query ` ```tasks ``` ` contra todo el vault y devuelve los resultados como datos (no HTML), para que el llamador los renderice con su propio estilo. Cada tarea incluye `dependsOnTasks` (descripción/ubicación de cada tarea a la que depende, `⛔`) y `blocking` (la inversa: qué tareas dependen de esta), ambas resueltas contra todo el vault — para que el llamador pueda mostrar esa información (p. ej. un popup al pasar el cursor) sin una segunda consulta.
