...
function rewriteTaskCheckboxLine(line: string): string {
...
function rewriteRawTaskLines(src: string): string {
...
function tagInlineRule(state: any, silent: boolean): boolean {
    const pos = state.pos;
...
function registerRawTaskLineStyling(md: any): void {
    ...
    md.renderer.render = (tokens: any[], options: any, env: any) => decodeIconAndStrikeMarkers(originalRender(tokens, options, env));
    ...
    md.renderer.renderInline = (tokens: any[], options: any, env: any) =>
        decodeIconAndStrikeMarkers(originalRenderInline(tokens, options, env));
}
