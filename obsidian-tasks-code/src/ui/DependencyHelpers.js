...
exports.descriptionAdjustedForDependencySearch = descriptionAdjustedForDependencySearch;
exports.searchForCandidateTasksForDependency = searchForCandidateTasksForDependency;
...
function descriptionAdjustedForDependencySearch(task) {
    return GlobalFilter_1.GlobalFilter.getInstance().removeAsWordFrom(task.description);
}
function searchDescriptionWithoutTags(query, allTasks) {
    if (query === '') {
        return allTasks;
    }
    const preparedSearch = (0, obsidian_1.prepareSimpleSearch)(query);
...
function searchForCandidateTasksForDependency(search, allTasks, task, blockedBy, blocking) {
    ...
    results = results.filter((item) => {
        ...
        const sameTask = item.description === task?.description &&
