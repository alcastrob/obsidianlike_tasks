...
const TaskLineRenderer_1 = require("../Renderer/TaskLineRenderer");
...
class InlineRenderer {
     ...
     */
    async _markdownPostProcessor(element, context) {
        ...
        const childComponent = new obsidian_1.MarkdownRenderChild(element);
        ...
        const renderedElements = element.findAll('.task-list-item').filter((taskItem) => {
            ...
            return GlobalFilter_1.GlobalFilter.getInstance().includedIn(firstLineText);
        });
        ...
        const tasksFile = new TasksFile_1.TasksFile(path, {}, file);
        ...
        for (let lineNumber = section.lineStart; lineNumber <= section.lineEnd; lineNumber++) {
            ...
            const task = Task_1.Task.fromLine({
                line,
                taskLocation: new TaskLocation_1.TaskLocation(tasksFile, lineNumber, section.lineStart, sectionIndex, precedingHeader),
        ...
        for (let sectionIndex = 0; sectionIndex < renderedElements.length; sectionIndex++) {
            ...
            const dataLine = renderedElement.getAttr('data-line') ?? '0';
            const taskIndex = Number.parseInt(dataLine, 10);
            const taskElement = (0, TaskLineRenderer_1.createAndAppendElement)('li', element);
            ...
            (0, TaskLineRenderer_1.reconcileReplacementTask)(renderedElement, taskElement);
        }
    }
}
