...
exports.Cache = exports.State = void 0;
...
class Cache {
    constructor({ metadataCache, vault, workspace, events, }) {
        this.logger = logging_1.logging.getLogger('tasks.Cache');
        this.notifySubscribersDebounced = (0, obsidian_1.debounce)(() => this.notifySubscribersNotDebounced(), 100, // Long enough to prevent successive redraws slowing performance; short enough for edits via context menus to appear snappy
        true);
...
`;
            ...
            if (this.state === State.Initializing) {
                new obsidian_2.Notice(msg, 10000);
            }
        };
        ...
        this.tasksMutex = new async_mutex_1.Mutex();
    ...
    subscribeToCache() {
        ...
        const changedEventReference = this.metadataCache.on('changed', async (file) => {
    ...
    subscribeToVault() {
        ...
        const { useFilenameAsScheduledDate } = (0, Settings_1.getSettings)();
        const createdEventReference = this.vault.on('create', async (file) => {
            if (!(file instanceof obsidian_2.TFile)) {
                return;
            }
        ...
        const deletedEventReference = this.vault.on('delete', async (file) => {
            if (!(file instanceof obsidian_2.TFile)) {
                return;
            }
        ...
        const renamedEventReference = this.vault.on('rename', async (file, oldPath) => {
            if (!(file instanceof obsidian_2.TFile)) {
                return;
            }
            ...
            await this.tasksMutex.runExclusive(() => {
                ...
                const tasksFile = new TasksFile_1.TasksFile(file.path, fileCache ?? undefined);
                const fallbackDate = new Lazy_1.Lazy(() => DateFallback_1.DateFallback.fromPath(file.path));
                this.tasks = this.tasks.map((task) => {
                    ...
                    if (useFilenameAsScheduledDate) {
                        return DateFallback_1.DateFallback.updateTaskPath(task, taskLocation, fallbackDate.value);
                    }
    ...
    loadVault() {
        ...
        return this.tasksMutex.runExclusive(async () => {
            const measureLoad = new PerformanceTracker_1.PerformanceTracker(`Loading vault with global filter '${GlobalFilter_1.GlobalFilter.getInstance().get()}'`);
            measureLoad.start();
            this.state = State.Initializing;
            this.logger.debug('Cache.loadVault(): state = Initializing');
            await Promise.all(this.vault.getMarkdownFiles().map((file) => {
    ...
    async indexFile(file) {
        ...
        if (listItems !== undefined) {
            ...
            newTasks = this.getTasksFromFileContent(new TasksFile_1.TasksFile(file.path, fileCache), fileContent, listItems, this.reportTaskParsingErrorToUser, this.logger);
        }
        // If there are no changes in any of the tasks, there's
        // nothing to do, so just return.
        if (ListItem_1.ListItem.listsAreIdentical(oldTasks, newTasks)) {
    ...
    getTasksFromFileContent(tasksFile, fileContent, listItems, errorReporter, logger) {
        const fileParser = new FileParser_1.FileParser(tasksFile, fileContent, listItems, logger, errorReporter);
