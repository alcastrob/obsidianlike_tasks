...
exports.replaceTaskWithTasks = exports.initializeFile = void 0;
exports.getTaskLineAndFile = getTaskLineAndFile;
exports.findLineNumberOfTaskToToggle = findLineNumberOfTaskToToggle;
const obsidian_1 = require("obsidian");
const GlobalFilter_1 = require("../Config/GlobalFilter");
const MockDataCreator_1 = require("../lib/MockDataCreator");
...
function getFileLogger() {
    ...
    return logging_1.logging.getLogger('tasks.File');
}
const initializeFile = ({ metadataCache: newMetadataCache, vault: newVault, workspace: newWorkspace, }) => {
 ...
 */
const replaceTaskWithTasks = async ({ originalTask, newTasks, }) => {
    ...
    (0, LogTasksHelper_1.logStartOfTaskEdit)(logger, codeLocation, originalTask);
...
exports.replaceTaskWithTasks = replaceTaskWithTasks;
/**
...
function errorAndNotice(message) {
    ...
    new obsidian_1.Notice(message, 15000);
}
function warnAndNotice(message) {
    console.warn(message);
    new obsidian_1.Notice(message, 10000);
}
 ...
 */
const tryRepetitive = async ({ originalTask, newTasks, vault, metadataCache, workspace, previousTries, }) => {
    const logger = getFileLogger();
 ...
 */
async function getTaskAndFileLines(task, vault) {
    ...
    if (taskLineNumber === undefined) {
        ...
        if (logDataForMocking) {
            ...
            (0, MockDataCreator_1.saveMockDataForTesting)(task, fileLines, listItemsCache);
        }
...
async function getTaskLineAndFile(task, vault) {
    try {
...
function tryFindingLineNumberFromTaskSectionInfo(originalTask, fileLines, listItemsCache, errorLoggingFunction) {
    ...
    for (const listItemCache of listItemsCache) {
        ...
        if (GlobalFilter_1.GlobalFilter.getInstance().includedIn(line)) {
            if (sectionIndex === originalTask.taskLocation.sectionIndex) {
                if (line === originalTask.originalMarkdown) {
                    ...
                    errorLoggingFunction(`Tasks: Unable to find task in file ${originalTask.taskLocation.path}.
Expected task:
${originalTask.originalMarkdown}
Found task:
${line}`);
