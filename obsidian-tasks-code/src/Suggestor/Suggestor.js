...
exports.showDependencySuggestionsDefault = exports.DEFAULT_MAX_GENERIC_SUGGESTIONS = void 0;
exports.makeDefaultSuggestionBuilder = makeDefaultSuggestionBuilder;
exports.matchIfCursorInRegex = matchIfCursorInRegex;
exports.lastOpenBracket = lastOpenBracket;
exports.onlySuggestIfBracketOpen = onlySuggestIfBracketOpen;
exports.canSuggestForLine = canSuggestForLine;
const DateParser_1 = require("../DateTime/DateParser");
const DateAbbreviations_1 = require("../DateTime/DateAbbreviations");
...
const DependencyHelpers_1 = require("../ui/DependencyHelpers");
 ...
 * for production code.
 */
exports.DEFAULT_MAX_GENERIC_SUGGESTIONS = 5;
// Set default value for production to off, temporarily. It will be turned on in tests.
exports.showDependencySuggestionsDefault = true;
globalThis.SHOW_DEPENDENCY_SUGGESTIONS = exports.showDependencySuggestionsDefault;
function includeDependencySuggestions(canSaveEdits) {
    return globalThis.SHOW_DEPENDENCY_SUGGESTIONS && canSaveEdits;
}
function makeDefaultSuggestionBuilder(symbols, maxGenericSuggestions /** See {@link DEFAULT_MAX_GENERIC_SUGGESTIONS} */, dataviewMode) {
     ...
     */
    return (line, cursorPos, settings, allTasks, canSaveEdits, taskToSuggestFor) => {
        let suggestions = [];
        ...
        if (includeDependencySuggestions(canSaveEdits)) {
            ...
            suggestions = suggestions.concat(addDependsOnSuggestions(symbols.dependsOnSymbol, allTasks, parameters, taskToSuggestFor));
        }
        // add on completion suggestions if relevant
        suggestions = suggestions.concat(addOnCompletionOptionSuggestions(symbols.onCompletionSymbol, maxGenericSuggestions, parameters));
 ...
 */
function addTaskPropertySuggestions(symbols, canSaveEdits, parameters) {
    ...
    if (matchingSuggestions.length === 0 && parameters.settings.autoSuggestMinMatch === 0)
        return genericSuggestions;
    return matchingSuggestions;
}
function addField(genericSuggestions, line, dateSymbol, fieldName) {
...
function addPrioritySuggestions(genericSuggestions, symbols, parameters) {
    const hasPriority = (line) => Object.values(symbols.prioritySymbols).some((value) => value.length > 0 && line.includes(value));
...
function addTaskLifecycleDateSuggestions(genericSuggestions, symbols, parameters) {
    // This will eventually also support Done and Cancelled dates
    if (!parameters.line.includes(symbols.createdDateSymbol)) {
        const parsedDate = DateParser_1.DateParser.parseDate('today', true);
        const formattedDate = parsedDate.format(TaskRegularExpressions_1.TaskRegularExpressions.dateFormat);
        genericSuggestions.push({
...
function filterGeneralSuggestionsForWordAtCursor(genericSuggestions, parameters) {
    ...
    if (wordMatch && wordMatch.length > 0) {
        ...
        if (wordUnderCursor.length >= Math.max(1, parameters.settings.autoSuggestMinMatch)) {
            ...
            for (const filtered of filteredSuggestions) {
                const insertSkipValue = parameters.dataviewMode &&
...
function defaultExtractor(symbol, suggestionText) {
    const displayText = `${suggestionText}`;
    const appendText = `${symbol} ${suggestionText}`;
    return { displayText, appendText };
}
function dateExtractor(symbol, date) {
    const parsedDate = DateParser_1.DateParser.parseDate(date, true);
    const formattedDate = `${parsedDate.format(TaskRegularExpressions_1.TaskRegularExpressions.dateFormat)}`;
 ...
 */
function addDatesSuggestions(datePrefixRegex, maxGenericSuggestions, parameters) {
    const genericSuggestions = [
        'today',
        'tomorrow',
        'Sunday',
    ...
    if (dateMatch && dateMatch.length >= 2) {
        ...
        const possibleDate = dateString && dateString.length > 1 ? DateParser_1.DateParser.parseDate((0, DateAbbreviations_1.doAutocomplete)(dateString), true) : null;
        if (possibleDate?.isValid()) {
            ...
            const absoluteDate = possibleDate.format(TaskRegularExpressions_1.TaskRegularExpressions.dateFormat);
 ...
 */
function addRecurrenceValueSuggestions(recurrenceSymbol, parameters) {
    const genericSuggestions = [
        'every',
        'every day',
        'every week',
    ...
    if (recurrenceMatch && recurrenceMatch.length >= 2) {
        ...
        if (recurrenceString.length < parameters.settings.autoSuggestMinMatch)
            return [];
        if (recurrenceString.length > 0) {
            ...
            const parsedRecurrence = Recurrence_1.Recurrence.fromText({
        ...
        let genericMatches = filterGenericSuggestions(genericSuggestions, recurrenceString, maxGenericDateSuggestions, false);
        ...
        const extractor = (recurrencePrefix, match) => {
 ...
 */
function addOnCompletionOptionSuggestions(symbol, maxGenericSuggestions, parameters) {
...
function addIDSuggestion(idSymbol, allTasks, parameters) {
    const results = [];
    const idRegex = new RegExp(`(${idSymbol})\\s*(${DefaultTaskSerializer_1.taskIdRegex.source})?`, 'ug');
    const idMatch = matchIfCursorInRegex(idRegex, parameters);
    if (idMatch && idMatch[0].trim().length <= idSymbol.length) {
        const ID = (0, TaskDependency_1.generateUniqueId)(allTasks.map((task) => task.id));
        results.push({
            suggestionType: 'match',
 ...
 * of what the user is typing.
 */
function addDependsOnSuggestions(dependsOnSymbol, allTasks, parameters, taskToSuggestFor) {
    const results = [];
    ...
    const charactersExcludedFromDescriptionSearch = parameters.dataviewMode
        ? (0, RegExpTools_1.escapeRegExp)('()[]')
        : (0, DefaultTaskSerializer_1.allTaskPluginEmojis)();
    const dependsOnRegex = new RegExp(`(${dependsOnSymbol})([0-9a-zA-Z-_ ^,]*,)*([^,${charactersExcludedFromDescriptionSearch}]*)`, 'ug');
    ...
    if (dependsOnMatch && dependsOnMatch.length >= 1) {
        ...
        if (newTaskToAppend.length >= parameters.settings.autoSuggestMinMatch) {
            const genericMatches = (0, DependencyHelpers_1.searchForCandidateTasksForDependency)(newTaskToAppend.trim(), allTasks, taskToSuggestFor, [], blockingTasks);
 ...
 */
function filterGenericSuggestions(genericSuggestions, typedText, maxGenericSuggestions, matchAllIfNoMatches) {
    const minMatch = 1;
    let genericMatches = genericSuggestions
        .filter((value) => typedText && typedText.length >= minMatch && value.toLowerCase().includes(typedText.toLowerCase()))
 ...
 */
function constructSuggestions(parameters, symbolAndTypedTextMatch, genericMatches, extractor, results) {
 ...
 */
function matchIfCursorInRegex(r, parameters) {
    const matches = parameters.line.matchAll(r);
    const cursorPos = parameters.cursorPos;
    for (const match of matches) {
        if (match?.index && match.index < cursorPos && cursorPos <= match.index + match[0].length)
            return match;
    }
}
/**
...
function onlySuggestIfBracketOpen(fn, brackets) {
    return (line, cursorPos, settings, taskToSuggestFor, allTasks) => {
...
function canSuggestForLine(line, cursor, editor) {
    const lineHasGlobalFilter = GlobalFilter_1.GlobalFilter.getInstance().includedIn(line);
...
function editorIsRequestingSuggest(editor, cursor, lineHasGlobalFilter) {
    return editor?.editorComponent?.showTasksPluginAutoSuggest?.(cursor, editor, lineHasGlobalFilter);
}
/**
 * Return true if:
...
function cursorIsInTaskLineDescription(line, cursorPosition) {
    ...
    const components = Task_1.Task.extractTaskComponents(line);
    if (!components) {
        // It is not a task line, that is, it is not a list item with a checkbox:
        return false;
    }
    if (components.status.type === StatusConfiguration_1.StatusType.NON_TASK) {
...
function calculateSkipValueForMatch(match, parameters) {
