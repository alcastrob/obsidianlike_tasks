...
class EditorSuggestor extends obsidian_1.EditorSuggest {
    ...
    onTrigger(cursor, editor, _file) {
        ...
        if (!(0, Suggestor_1.canSuggestForLine)(line, cursor, editor)) {
            return null;
        }
        if (this.grabSuggestions(editor, _file, line).length === 0)
            return null;
        return {
    ...
    grabSuggestions(editor, file, line) {
        const currentCursor = editor.getCursor();
        const allTasks = this.plugin.getTasks();
        const taskToSuggestFor = allTasks.find((task) => task.taskLocation.path == file.path && task.taskLocation.lineNumber == currentCursor.line);
        ...
        return ((0, Settings_1.getUserSelectedTaskFormat)().buildSuggestions?.(line, currentCursor.ch, this.settings, allTasks, canSaveEdits, taskToSuggestFor) ?? []);
    }
    getMarkdownFileInfo(editor) {
        // @ts-expect-error: TS2339: Property cm does not exist on type Editor
        return editor.cm.state.field(obsidian_1.editorInfoField);
    }
    canSaveEdits(markdownFileInfo) {
        return markdownFileInfo instanceof obsidian_1.MarkdownView;
    }
    renderSuggestion(value, el) {
        el.setText(value.displayText);
    }
    async selectSuggestion(value, _evt) {
        ...
        if (value.suggestionType === 'empty') {
            ...
            editor?.cm?.contentDOM?.dispatchEvent(eventClone);
            return;
        }
        if (value.taskItDependsOn != null) {
            const newTask = (0, TaskDependency_1.ensureTaskHasId)(value.taskItDependsOn, this.plugin.getTasks().map((task) => task.id));
            ...
            if (value.taskItDependsOn !== newTask) {
                ...
                if (value.context.file.path == newTask.path) {
                    ...
                    const markdownInEditor = value.context.editor.getRange(start, end);
...
`;
                    ...
                    await (0, File_1.replaceTaskWithTasks)({ originalTask: value.taskItDependsOn, newTasks: newTask });
                }
            }
        }
...
exports.EditorSuggestor = EditorSuggestor;
//# sourceMappingURL=EditorSuggestorPopup.js.map