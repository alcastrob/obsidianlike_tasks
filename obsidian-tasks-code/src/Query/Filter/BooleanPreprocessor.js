...
exports.BooleanPreprocessor = void 0;
...
class BooleanPreprocessor {
    ...
    static splitLine(line, delimiters) {
        ...
        //   ')AND  NOT('
        //   ')  AND  NOT  ('
        const binaryOperatorsRegex = new RegExp('(' + delimiters.closeFilter + '\\s*(?:AND|OR|AND +NOT|OR +NOT|XOR)\\s*' + delimiters.openFilter + ')');
        ...
        const openingDelimitersAndSpacesAtStartRegex = new RegExp('(^' + (0, BooleanDelimiters_1.anyOfTheseChars)(delimiters.openFilterChars + ' ') + '*)');
        const closingDelimitersAndSpacesAtEndRegex = new RegExp('(' + (0, BooleanDelimiters_1.anyOfTheseChars)(delimiters.closeFilterChars + ' ') + '*$)');
    ...
    static getFiltersAndSimplifiedLine(parts, delimiters) {
        ...
        const filters = {}; // To store filter placeholders and their corresponding text
        ...
        if (openChars != '"' && openChars != '(') {
            const openDelimiter = new RegExp((0, BooleanDelimiters_1.anyOfTheseChars)(openChars), 'g');
            simplifiedLine = simplifiedLine.replace(openDelimiter, '(');
            const closeChars = delimiters.closeFilterChars;
            const closeDelimiter = new RegExp((0, BooleanDelimiters_1.anyOfTheseChars)(closeChars), 'g');
    ...
    static isAFilter(part, delimiters) {
        ...
        const onlySpacesAndParentheses = new RegExp('^' + (0, BooleanDelimiters_1.anyOfTheseChars)(' ' + delimiters.openAndCloseFilterChars) + '+$');
        const binaryOperatorAndParentheses = new RegExp('^ *' + delimiters.closeFilter + ' *(AND|OR|XOR) *' + delimiters.openFilter + ' *$');
...
exports.BooleanPreprocessor = BooleanPreprocessor;
//# sourceMappingURL=BooleanPreprocessor.js.map