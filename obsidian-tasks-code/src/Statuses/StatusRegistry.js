...
const HTMLCharacterEntities_1 = require("../lib/HTMLCharacterEntities");
...
class StatusRegistry {
    ...
    static copyStatusWithNewName(s, newName) {
        const statusConfiguration = new StatusConfiguration_1.StatusConfiguration(s.symbol, newName, s.nextStatusSymbol, s.availableAsCommand, s.type);
        return new Status_1.Status(statusConfiguration);
    }
    /**
...
`;
    ...
    addEdgeIfNotToInternal(uniqueStatuses, nextStatus, edges, index, isForReccurenceOverride) {
        ...
        const nextStatusIsNotInternal = nextStatus.type !== StatusConfiguration_1.StatusType.EMPTY;
    ...
    getMermaidNodeLabel(status, includeDetails) {
        const statusName = (0, HTMLCharacterEntities_1.htmlEncodeString)(status.name);
        const statusType = status.type;
        if (includeDetails) {
            const statusSymbol = (0, HTMLCharacterEntities_1.htmlEncodeCharacter)(status.symbol);
            const statusNextStatusSymbol = (0, HTMLCharacterEntities_1.htmlEncodeCharacter)(status.nextStatusSymbol);
