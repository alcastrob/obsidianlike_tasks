...
describe('CreateOrEditTaskParser - testing edited task if line is saved unchanged', () => {
    ...
    ])('line loaded into "Create or edit task" command: "%s"', (line, expectedResult, globalFilter) => {
...
describe('CreateOrEditTaskParser - task recognition', () => {
    ...
    it('should recognize task details without global filter', () => {
        ...
        const taskLine = '- [ ] without global filter but with all the info ⏬ 🔁 every 2 days ➕ 2022-03-10 🛫 2022-01-31 ⏳ 2023-06-13 📅 2024-12-10 ✅ 2023-06-22';
        const path = 'a/b/c.md';
        ...
        expect(task.priority).toStrictEqual(Priority_1.Priority.Lowest);
        expect(task.recurrenceRule).toStrictEqual('every 2 days');
        expect(task.createdDate).toEqualMoment((0, moment_1.default)('2022-03-10'));
        expect(task.startDate).toEqualMoment((0, moment_1.default)('2022-01-31'));
        expect(task.scheduledDate).toEqualMoment((0, moment_1.default)('2023-06-13'));
        expect(task.dueDate).toEqualMoment((0, moment_1.default)('2024-12-10'));
        expect(task.doneDate).toEqualMoment((0, moment_1.default)('2023-06-22'));
    });
});
describe('CreateOrEditTaskParser - created date', () => {
    ...
    ])('line loaded into "Create or edit task" command: "%s"', (line, expectedTaskLine, expectedCreatedDate) => {
        (0, Settings_1.updateSettings)({ setCreatedDate: true });
        const path = 'a/b/c.md';
        const task = (0, CreateOrEditTaskParser_1.taskFromLine)({ line, path });
        expect(task.toFileLineString()).toStrictEqual(expectedTaskLine);
        expect(task.createdDate).toEqualMoment((0, moment_1.default)(expectedCreatedDate));
    });
    ...
    it('should add created date if adding the global filter', () => {
        ...
        expect(task.createdDate).toEqualMoment((0, moment_1.default)('2023-09-17'));
    });
});
//# sourceMappingURL=CreateOrEditTaskParser.test.js.map