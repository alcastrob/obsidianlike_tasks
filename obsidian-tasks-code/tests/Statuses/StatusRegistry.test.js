...
describe('StatusRegistry', () => {
    ...
    it('should create a new instance and populate default status symbols', () => {
        ...
        expect(doneStatus.symbol).toEqual(Status_1.Status.DONE.symbol);
        expect(statusRegistry.bySymbol('x').symbol).toEqual(Status_1.Status.DONE.symbol);
        expect(statusRegistry.bySymbol('').symbol).toEqual(Status_1.Status.EMPTY.symbol);
        expect(statusRegistry.bySymbol(' ').symbol).toEqual(Status_1.Status.TODO.symbol);
        expect(statusRegistry.bySymbol('-').symbol).toEqual(Status_1.Status.CANCELLED.symbol);
        expect(statusRegistry.bySymbol('/').symbol).toEqual(Status_1.Status.IN_PROGRESS.symbol);
    ...
    it('should allow setting the entire set of statuses', () => {
        ...
        const statusRegistry = new StatusRegistry_1.StatusRegistry();
        const statuses = [
            new StatusConfiguration_1.StatusConfiguration('Q', 'Question', 'A', false, StatusConfiguration_1.StatusType.NON_TASK),
            new StatusConfiguration_1.StatusConfiguration('A', 'Answer', 'Q', false, StatusConfiguration_1.StatusType.NON_TASK),
        ];
        // Act
    ...
    it('should return empty status for lookup by unknown symbol with bySymbol()', () => {
        ...
        // Assert
        expect(result).toEqual(Status_1.Status.EMPTY);
    });
    ...
    it('should allow lookup of next status for a status', () => {
        ...
        const statusA = new Status_1.Status(new StatusConfiguration_1.StatusConfiguration('a', 'A', 'b', false));
        const statusB = new Status_1.Status(new StatusConfiguration_1.StatusConfiguration('b', 'B', 'c', false));
        const statusC = new Status_1.Status(new StatusConfiguration_1.StatusConfiguration('c', 'C', 'd', false));
        const statusD = new Status_1.Status(new StatusConfiguration_1.StatusConfiguration('d', 'D', 'a', false));
        // Act
    ...
    it('should return EMPTY if next status does not exist', () => {
        const statusRegistry = new StatusRegistry_1.StatusRegistry();
        const status = new Status_1.Status(new StatusConfiguration_1.StatusConfiguration('P', 'Pro', 'C', false));
        const nextStatus = statusRegistry.getNextStatus(status);
        expect(nextStatus.type).toEqual(StatusConfiguration_1.StatusType.EMPTY);
    });
    it('should construct a TODO on request if next status does not exist', () => {
        ...
        expect(nextStatus.type).toEqual(StatusConfiguration_1.StatusType.TODO);
    });
    it('should handle adding custom StatusConfiguration', () => {
        // Arrange
        const statusRegistry = new StatusRegistry_1.StatusRegistry();
        const statusConfiguration = new StatusConfiguration_1.StatusConfiguration('a', 'A', 'b', false);
        statusRegistry.add(statusConfiguration);
        // Assert
        const status2 = statusRegistry.bySymbol('a');
        expect(status2).toStrictEqual(new Status_1.Status(statusConfiguration));
    });
    it('should not modify an added custom Status', () => {
        // Arrange
        const statusRegistry = new StatusRegistry_1.StatusRegistry();
        const status = new Status_1.Status(new StatusConfiguration_1.StatusConfiguration('a', 'A', 'b', false));
    ...
    it('should find unknown statuses from tasks in the vault, sorted by symbol', () => {
        ...
        expect(registry.bySymbol('!').type).toEqual(StatusConfiguration_1.StatusType.EMPTY);
        expect(registry.bySymbol('X').type).toEqual(StatusConfiguration_1.StatusType.EMPTY);
        expect(registry.bySymbol('d').type).toEqual(StatusConfiguration_1.StatusType.EMPTY);
        const allStatuses = [
            new Status_1.Status(new StatusConfiguration_1.StatusConfiguration('!', 'Unknown', 'X', false, StatusConfiguration_1.StatusType.TODO)),
            new Status_1.Status(new StatusConfiguration_1.StatusConfiguration('X', 'Unknown', '!', false, StatusConfiguration_1.StatusType.DONE)),
            new Status_1.Status(new StatusConfiguration_1.StatusConfiguration('d', 'Unknown', '!', false, StatusConfiguration_1.StatusType.IN_PROGRESS)),
            ...
            new Status_1.Status(new StatusConfiguration_1.StatusConfiguration('-', 'Unknown', '!', false, StatusConfiguration_1.StatusType.IN_PROGRESS)),
        ];
        // Act
        ...
        expect(s1.type).toEqual(StatusConfiguration_1.StatusType.TODO);
        expect(s1.name).toEqual('Unknown (!)');
        s1 = unknownStatuses[1];
        expect(s1.type).toEqual(StatusConfiguration_1.StatusType.IN_PROGRESS);
        expect(s1.name).toEqual('Unknown (d)');
        s1 = unknownStatuses[2];
        expect(s1.type).toEqual(StatusConfiguration_1.StatusType.DONE);
    ...
    describe('mermaid diagrams', () => {
        ...
        it('should encode symbols in mermaid diagrams when necessary', () => {
            ...
            const statusRegistry = new StatusRegistry_1.StatusRegistry();
            statusRegistry.clearStatuses();
            statusRegistry.add(new StatusConfiguration_1.StatusConfiguration('<', 'Todo <', '<', false, StatusConfiguration_1.StatusType.TODO));
            statusRegistry.add(new StatusConfiguration_1.StatusConfiguration('>', 'Todo >', '>', false, StatusConfiguration_1.StatusType.TODO));
            statusRegistry.add(new StatusConfiguration_1.StatusConfiguration('"', 'Todo "', '"', false, StatusConfiguration_1.StatusType.TODO));
            statusRegistry.add(new StatusConfiguration_1.StatusConfiguration('&', 'Todo &', '&', false, StatusConfiguration_1.StatusType.TODO));
            // Assert
    ...
    describe('toggling', () => {
        ...
        it('should allow task to toggle through standard transitions', () => {
            ...
            expect(task.status.symbol).toEqual(Status_1.Status.TODO.symbol);
            const toggledDone = task?.toggle()[0];
            expect(toggledDone?.status.symbol).toEqual(Status_1.Status.DONE.symbol);
            const toggledTodo = toggledDone?.toggle()[0];
            expect(toggledTodo?.status.symbol).toEqual(Status_1.Status.TODO.symbol);
        });
        it('should allow task to toggle from cancelled to todo', () => {
            ...
            // Assert
            expect(task).not.toBeNull();
            expect(task.status.symbol).toEqual(Status_1.Status.CANCELLED.symbol);
        ...
        it('should allow task to toggle through custom transitions', () => {
            ...
            const globalStatusRegistry = StatusRegistry_1.StatusRegistry.getInstance();
            const statusA = new Status_1.Status(new StatusConfiguration_1.StatusConfiguration('a', 'A', 'b', false));
            const statusB = new Status_1.Status(new StatusConfiguration_1.StatusConfiguration('b', 'B', 'c', false));
            const statusC = new Status_1.Status(new StatusConfiguration_1.StatusConfiguration('c', 'C', 'd', false));
            const statusD = new Status_1.Status(new StatusConfiguration_1.StatusConfiguration('d', 'D', 'a', false));
        ...
        it('should leave task valid if toggling to unknown status', () => {
            // Arrange
            const globalStatusRegistry = StatusRegistry_1.StatusRegistry.getInstance();
            const important = new StatusConfiguration_1.StatusConfiguration('!', 'Important', 'D', false);
    ...
    describe('toggling recurring', () => {
         ...
         */
        function checkToggleAndRecurrenceStatuses(statuses, initialStatusSymbol, expectedToggledStatus, expectedNextTaskStatus) {
            // Arrange
            const statusRegistry = new StatusRegistry_1.StatusRegistry();
            statusRegistry.set((0, StatusesTestHelpers_1.constructStatuses)(statuses));
...
//# sourceMappingURL=StatusRegistry.test.js.map