...
describe('StatusValidator', () => {
    ...
    function checkValidation(statusConfiguration, expectedMessages) {
    ...
    describe('validate StatusConfiguration', () => {
        it('should handle valid input correctly', () => {
            const config = new StatusConfiguration_1.StatusConfiguration('X', 'Completed', ' ', false);
            checkValidation(config, []);
        });
        it('should handle totally invalid input correctly', () => {
            const config = new StatusConfiguration_1.StatusConfiguration('Xxx', '', '', false);
            checkValidation(config, [
        ...
        it('should detect empty name', () => {
            const config = new StatusConfiguration_1.StatusConfiguration('X', '', ' ', false);
            checkValidation(config, ['Task Status Name cannot be empty.']);
        });
        // Check Symbol
        it('should detect empty symbol', () => {
            const config = new StatusConfiguration_1.StatusConfiguration('', 'Completed', ' ', false);
            checkValidation(config, ['Task Status Symbol cannot be empty.']);
        });
        it('should detect too-long symbol', () => {
            const config = new StatusConfiguration_1.StatusConfiguration('yyy', 'Completed', ' ', false);
        ...
        it('should detect too-long next symbol', () => {
            const config = new StatusConfiguration_1.StatusConfiguration('X', 'Completed', 'yyy', false);
    ...
    describe('validate symbol', () => {
        it('valid symbol', () => {
            const config = new StatusConfiguration_1.StatusConfiguration('X', 'Completed', 'c', false);
            expect(statusValidator.validateSymbol(config)).toStrictEqual([]);
        });
        it('invalid symbol', () => {
            const config = new StatusConfiguration_1.StatusConfiguration('XYZ', 'Completed', 'c', false);
            expect(statusValidator.validateSymbol(config)).toStrictEqual([
    ...
    describe('validate next symbol', () => {
        it('valid symbol', () => {
            const config = new StatusConfiguration_1.StatusConfiguration('c', 'Completed', 'X', false);
            expect(statusValidator.validateNextSymbol(config)).toStrictEqual([]);
        });
        it('invalid next symbol', () => {
            const config = new StatusConfiguration_1.StatusConfiguration('c', 'Completed', 'XYZ', false);
            expect(statusValidator.validateNextSymbol(config)).toStrictEqual([
    ...
    describe('validate symbol/type consistency for convention', () => {
        it('matches convention', () => {
            expect(statusValidator.validateSymbolTypeConventions(new StatusConfiguration_1.StatusConfiguration(' ', 'Any old name', 'x', false, StatusConfiguration_1.StatusType.TODO))).toEqual([]);
            expect(statusValidator.validateSymbolTypeConventions(new StatusConfiguration_1.StatusConfiguration('X', 'Any old name', ' ', false, StatusConfiguration_1.StatusType.DONE))).toEqual([]);
        });
        it('does not match convention', () => {
            expect(statusValidator.validateSymbolTypeConventions(new StatusConfiguration_1.StatusConfiguration(' ', 'Any old name', '!', false, StatusConfiguration_1.StatusType.IN_PROGRESS))).toEqual([
            ...
            expect(statusValidator.validateSymbolTypeConventions(new StatusConfiguration_1.StatusConfiguration(' ', 'Any old name', 'x', false, StatusConfiguration_1.StatusType.NON_TASK))).toEqual(["Status Type for symbol ' ': 'NON_TASK' is inconsistent with convention 'TODO'"]);
            expect(statusValidator.validateSymbolTypeConventions(new StatusConfiguration_1.StatusConfiguration('X', 'Name', '-', false, StatusConfiguration_1.StatusType.TODO))).toEqual([
                "Next Status Symbol for symbol 'X': '-' is inconsistent with convention ' '",
                "Status Type for symbol 'X': 'TODO' is inconsistent with convention 'DONE'",
            ]);
        });
...
//# sourceMappingURL=StatusValidator.test.js.map