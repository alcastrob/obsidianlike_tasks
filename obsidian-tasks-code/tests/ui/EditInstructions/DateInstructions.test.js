...
const DateInstructions_1 = require("../../../src/ui/EditInstructions/DateInstructions");
...
const taskWithNoDates = new TaskBuilder_1.TaskBuilder().build();
const taskDueToday = new TaskBuilder_1.TaskBuilder().dueDate(today).build();
const taskDueTomorrow = new TaskBuilder_1.TaskBuilder().dueDate(tomorrow).build();
describe('SetTaskDate', () => {
    it('should provide information to set up a menu item for due date', () => {
        // Arrange
        const instruction = new DateInstructions_1.SetTaskDate('dueDate', new Date(today));
        // Assert
    ...
    it('should create a menu item with a custom display name', () => {
        // Arrange
        const instruction = new DateInstructions_1.SetTaskDate('dueDate', new Date(today), 'Apple Sauce');
        // Assert
        expect(instruction.instructionDisplayName()).toEqual('Apple Sauce');
    });
    it('should edit the date', () => {
        // Arrange
        const instruction = new DateInstructions_1.SetTaskDate('dueDate', new Date(tomorrow));
        // Apply
        const newTasks = instruction.apply(taskWithNoDates);
        // Assert
        expect(newTasks.length).toEqual(1);
        expect(newTasks[0].dueDate).toEqualMoment((0, moment_1.default)(tomorrow));
    });
...
describe('SetRelativeTaskDate', () => {
    function testSetRelativeTaskDate(task, dateFieldToEdit, amount, timeUnit, expectedTitle, expectedNewDate) {
        // Arrange
        const instruction = new DateInstructions_1.SetRelativeTaskDate(dateFieldToEdit, task, amount, timeUnit);
        // Apply
    ...
    it('should postpone a task with a due date', () => {
        ...
        testSetRelativeTaskDate(taskDueToday, 'dueDate', 1, 'day', expectedTitle, (0, moment_1.default)(tomorrow));
    });
    it("should postpone a task without a due date, based on today's date", () => {
        const expectedTitle = 'Start in 2 weeks, on Tue 15th Oct';
        testSetRelativeTaskDate(taskWithNoDates, 'startDate', 2, 'weeks', expectedTitle, (0, moment_1.default)('2024-10-15'));
    });
    it('should set a done date', () => {
        const expectedTitle = 'Done today, on Tue 1st Oct';
        testSetRelativeTaskDate(taskWithNoDates, 'doneDate', 0, 'days', expectedTitle, (0, moment_1.default)(today));
    });
});
describe('RemoveTaskDate', () => {
    it('should provide information to set up a menu item for removing start date', () => {
        const instruction = new DateInstructions_1.RemoveTaskDate('startDate', taskWithNoDates);
    ...
    it('should refuse to remove scheduled date from task with inferred scheduled date', () => {
        ...
        const instruction = new DateInstructions_1.RemoveTaskDate('scheduledDate', task);
    ...
    it('should not edit task if already has no value in field being removed', () => {
        ...
        const task = new TaskBuilder_1.TaskBuilder()
            ...
            .startDate(today)
            .build();
        const instruction = new DateInstructions_1.RemoveTaskDate('cancelledDate', task);
        // Apply
        const newTasks = instruction.apply(task);
        // Assert
        expect(newTasks.length).toEqual(1);
    ...
    it('should create a new task with Due date removed', () => {
        const instruction = new DateInstructions_1.RemoveTaskDate('dueDate', taskDueToday);
...
describe('DateInstruction lists', () => {
    function applyAllInstructions(field, currentFieldValue, datesFunction) {
        const task = new Task_1.Task({ ...new TaskBuilder_1.TaskBuilder().build(), ...currentFieldValue });
        ...
        return ('\n' +
            instructions
                ...
                if (label === MenuDividerInstruction_1.SEPARATOR_INSTRUCTION_DISPLAY_NAME) {
                    return `${checkMark} ${label}`;
                }
                const newTasks = instruction.apply(task);
                expect(newTasks.length).toEqual(1);
                const newDate = newTasks[0][field];
                return `${checkMark} ${label} => ${new TasksDate_1.TasksDate(newDate).formatAsDate('No Date')}`;
            })
                .join('\n'));
    }
    describe('happens dates', () => {
        it('should offer future dates for task due yesterday', () => {
            const allAppliedToTask = applyAllInstructions(TaskLayoutOptions_1.TaskLayoutComponent.DueDate, { dueDate: window.moment(yesterday) }, DateInstructions_1.allHappensDateInstructions);
        ...
        it('should offer future dates for task due today', () => {
            const allAppliedToTask = applyAllInstructions(TaskLayoutOptions_1.TaskLayoutComponent.ScheduledDate, { scheduledDate: window.moment(today) }, DateInstructions_1.allHappensDateInstructions);
            expect(allAppliedToTask).toMatchInlineSnapshot(`
                "
        ...
        it('should offer future dates for task due tomorrow', () => {
            const allAppliedToTask = applyAllInstructions(TaskLayoutOptions_1.TaskLayoutComponent.StartDate, { startDate: window.moment(tomorrow) }, DateInstructions_1.allHappensDateInstructions);
            expect(allAppliedToTask).toMatchInlineSnapshot(`
                "
    ...
    describe('lifecycle dates', () => {
        it('should offer past dates for task due yesterday', () => {
            const allAppliedToTask = applyAllInstructions(TaskLayoutOptions_1.TaskLayoutComponent.CreatedDate, { createdDate: window.moment(yesterday) }, DateInstructions_1.allLifeCycleDateInstructions);
            expect(allAppliedToTask).toMatchInlineSnapshot(`
                "
        ...
        it('should offer past dates for task due today', () => {
            const allAppliedToTask = applyAllInstructions(TaskLayoutOptions_1.TaskLayoutComponent.DoneDate, { doneDate: window.moment(today) }, DateInstructions_1.allLifeCycleDateInstructions);
            expect(allAppliedToTask).toMatchInlineSnapshot(`
                "
        ...
        it('should offer past dates for task due tomorrow', () => {
            const allAppliedToTask = applyAllInstructions(TaskLayoutOptions_1.TaskLayoutComponent.CancelledDate, { cancelledDate: window.moment(tomorrow) }, DateInstructions_1.allLifeCycleDateInstructions);
            expect(allAppliedToTask).toMatchInlineSnapshot(`
                "
...
//# sourceMappingURL=DateInstructions.test.js.map