...
const overdue = new TasksDate_1.TasksDate((0, moment_1.default)('2023-06-10'));
const today = new TasksDate_1.TasksDate((0, moment_1.default)('2023-06-11'));
const future = new TasksDate_1.TasksDate((0, moment_1.default)('2023-06-12'));
const invalid = new TasksDate_1.TasksDate((0, moment_1.default)('2023-02-31'));
...
describe('TasksDate', () => {
    it('should format valid dates', () => {
        ...
        const tasksDate = new TasksDate_1.TasksDate((0, moment_1.default)(date));
    ...
    it('should format null dates as empty string', () => {
        const tasksDate = new TasksDate_1.TasksDate(null);
    ...
    it('should format invalid dates meaningfully', () => {
        const tasksDate = new TasksDate_1.TasksDate((0, moment_1.default)('2023-12-32'));
    ...
    it('should confirm that sample dates really are sorted old-to-new', () => {
        ...
        const sampleDates = sampleDatesSortedByDate.map((values) => values[0]);
        const sampleDatesSorted = [...sampleDates].sort((a, b) => numericStringCompare(a, b));
        expect(sampleDatesSorted).toEqual(sampleDates);
    });
    it.each(sampleDatesSortedByDate)('should categorise dates for grouping, relative to today: on "%s" - expected "%s"', (date, expectedResult) => {
        const tasksDate = new TasksDate_1.TasksDate((0, moment_1.default)(date));
        expect(tasksDate.fromNow.groupText).toEqual(expectedResult);
    });
    it.each(sampleDatesSortedByDate)('should categorise dates for grouping, in correct order, in Spanish: on "%s"', (date, expectedResult) => {
        const extractSortOrder = (result) => {
        ...
        const tasksDate = new TasksDate_1.TasksDate((0, moment_1.default)(date).locale('es'));
    ...
    it('should sort group categories in ascending order by date', () => {
        ...
        const sampleDateGroupNames = sampleDatesSortedByDate.map((values) => values[1]);
    ...
    it('should categorise edge-case dates for grouping, relative to today', () => {
        expect(new TasksDate_1.TasksDate(null).fromNow.groupText).toEqual('');
        // Invalid dates always get put first
        expect(new TasksDate_1.TasksDate((0, moment_1.default)('1999-02-31')).fromNow.groupText).toEqual('%%0%% Invalid date');
        expect(new TasksDate_1.TasksDate((0, moment_1.default)('2023-02-31')).fromNow.groupText).toEqual('%%0%% Invalid date');
    });
    describe('language-aware fromNow() calculations', () => {
        ...
        describe('should calculate correct sort order for fromNow value in given language, for a date in two days', () => {
            ...
            it.each(locales)('%s locale', (loc, expectedFromNow) => {
                jest.setSystemTime(new Date(thisTime));
                const now = (0, moment_1.default)(taskTime);
                const tasksDate = new TasksDate_1.TasksDate(now.clone().locale(loc));
...
describe('TasksDate - postpone', () => {
    ...
    function checkDatePostponesTo(initialDate, amount, unitOfTime, expectedDate) {
        const tasksDate = new TasksDate_1.TasksDate((0, moment_1.default)(initialDate));
        const postponedDate = new TasksDate_1.TasksDate(tasksDate.postpone(unitOfTime, amount));
        expect(postponedDate.formatAsDate()).toEqual(expectedDate);
    }
    it('should not modify the original date when postponing', () => {
        function checkPostponingDoesNotModifyOriginalDate(initialDate, amount, unitOfTime) {
            // Arrange
            const initialTasksDate = new TasksDate_1.TasksDate((0, moment_1.default)(initialDate));
            // Act
            const postponed = new TasksDate_1.TasksDate(initialTasksDate.postpone(unitOfTime, amount));
            // Assert
    ...
    describe('visualise postpone behaviour', () => {
        function postponeMultipleDatesBy(amount, unitOfTime) {
...
//# sourceMappingURL=TasksDate.test.js.map