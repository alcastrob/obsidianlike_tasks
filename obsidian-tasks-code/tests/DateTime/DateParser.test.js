...
function testParsingeSingleDate(input, result) {
    const moment = DateParser_1.DateParser.parseDate(input);
    expect(moment.format(TaskRegularExpressions_1.TaskRegularExpressions.dateFormat)).toEqual(result);
}
function testParsingDateRange(input, expectedStart, expectedEnd) {
    // Act
    const result = DateParser_1.DateParser.parseDateRange(input);
    // Assert
    expect(result.start).toBeDefined();
    expect(result.end).toBeDefined();
    const startFmt = result.start.format(TaskRegularExpressions_1.TaskRegularExpressions.dateFormat);
    const endFmt = result.end.format(TaskRegularExpressions_1.TaskRegularExpressions.dateFormat);
...
describe('DateParser - single dates', () => {
    ...
    it('should return date at midnight', () => {
        ...
        const parsedDate = DateParser_1.DateParser.parseDate(dateToParse);
