...
function makeIncludes(...entries) {
    return Object.fromEntries(entries);
}
const tasksFile = (0, TasksFileHelpers_1.createTestTasksFile)('root/folder/stuff.md');
function createQuery(source, includes) {
    (0, Settings_1.updateSettings)({ presets: includes });
    const query = new Query_1.Query(source, tasksFile);
    expect(query.source).toEqual(source);
    return query;
}
function createValidQuery(source, includes) {
    const query = createQuery(source, includes);
    expect(query.error).toBeUndefined();
    return query;
}
function expectExpandedStatementToBe(statement, expandedLine) {
    expect(statement.anyPlaceholdersExpanded).toEqual(expandedLine);
}
function expectFiltersToBe(query, expectedFilterLines) {
...
describe('include tests', () => {
    ...
    it('should support nested include instructions', () => {
        ...
        // Force line break
        ['inside', 'not done'], ['out', 'preset inside\nhide edit button']);
    ...
    describe('multi-line placeholders inside includes', () => {
        const includes = makeIncludes(['two_lines', 'has due date\nhas created date'], ['two_lines_as_include', 'preset two_lines'], ['two_lines_as_placeholder', '{{preset.two_lines}}']);
    ...
    describe('placeholders inside placeholders', () => {
        ...
        ['this_path', 'path includes {{query.file.path}}'], ['this_folder', 'folder includes {{query.file.folder}}'], ['this_root', 'root includes {{query.file.root}}'], 
        // 2-level include:
...
return "Hello World";`;
    ...
    describe('includes inside Boolean combinations', () => {
        ...
        const includes = makeIncludes(
        // Force line break
        ['filter1', 'not done'], ['filter2', 'due 2025-05-01']);
...
describe('include - explain output', () => {
    describe('explain two levels of nested includes', () => {
        const includes = makeIncludes(['inside', '(happens this week) AND (starts before today)'], ['out', 'preset inside\nnot done']);
...
describe('include - error messages', () => {
    ...
    describe('use of non-existent include, when some includes are defined', () => {
        const includes = makeIncludes(['include2', 'task.due.format("YYYY")'], ['include1', 'sort by function task.lineNumber'], [
            'include3',
    ...
    describe('expanded text is invalid instruction', () => {
        const includes = makeIncludes(
        // Force line break
        ['inside', 'apple sauce'], ['out', 'preset inside']);
    ...
    describe('infinite recursion of instruction', () => {
        const includes = makeIncludes(['self_reference_1', '{{preset.self_reference_1}}'], ['self_reference_2', 'preset self_reference_2']);
...
describe('include settings tests', () => {
    ...
    it.each(Object.entries(Presets_1.defaultPresets))('should handle preset "%s"', (name, instructions) => {
        expect(name).toBeDefined();
        expect(instructions).toBeDefined();
        const query = new Query_1.Query(instructions, (0, TasksFileHelpers_1.createTestTasksFile)('anywhere.md'));
        expect(query.error).toBeUndefined();
    });
});
