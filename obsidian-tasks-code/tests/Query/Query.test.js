...
const namedFields = FilterParser_1.fieldCreators
    .map((creator) => {
    const field = creator();
    return { name: field.fieldName(), field };
})
    .sort((a, b) => a.name.localeCompare(b.name, undefined, { numeric: true }));
function sortInstructionLines(filters) {
    // Sort a copy of the array of filters.
    return [...filters].sort((a, b) => a.localeCompare(b, undefined, { numeric: true }));
}
function isValidQueryFilter(filter) {
    // Arrange
    const query = new Query_1.Query(filter, (0, TasksFileHelpers_1.createTestTasksFile)('anywhere.md'));
    // Assert
...
function lowerCaseFilterGaveExpectionInstruction(filter, instruction) {
...
function isValidQuerySort(filter) {
    // Arrange
...
function isValidQueryGroup(filter) {
    // Arrange
    const query = new Query_1.Query(filter);
    // Assert
    expect(query.error).toBeUndefined();
...
function isInvalidQueryInstruction(getQueryError, source, expectedErrorMessage) {
    expect(getQueryError(source)).toEqual(`${expectedErrorMessage}
Problem line: "${source}"`);
}
function isInvalidQueryInstructionLowerAndUpper(getQueryError, source, expectedErrorMessage = 'do not understand query') {
...
description includes \
    from a Continuation Line
        `;
        const query = new Query_1.Query(source, (0, TasksFileHelpers_1.createTestTasksFile)('test.md'));
        expect(query.error).toBeUndefined();
    ...
    const notValidWhenCapitalised = filters.filter((line) => ['preset ', '{{preset.'].some((prefix) => line.startsWith(prefix)));
    const notValidInBoolean = filters.filter((line) => ['preset '].some((prefix) => line.startsWith(prefix)));
    /**
    ...
    describe('should recognise every supported filter', () => {
        test.concurrent.each(filters)('recognises %j', (filter) => {
        ...
        function linesMatchingField(field) {
            return filters.filter((instruction) => {
                return (field.canCreateFilterForLine(instruction) &&
    ...
    describe('should not confuse a boolean query for any other single field', () => {
        ...
        const searchInfo = SearchInfo_1.SearchInfo.fromAllTasks([task]);
        test.concurrent.each(filters)('sub-query %j is recognized inside a boolean query', (filter) => {
    ...
    describe('should recognise every other instruction', () => {
        ...
        test.concurrent.each(filters)('recognises %j', (filter) => {
            // Arrange
            const query = new Query_1.Query(filter);
            const queryUpperCase = new Query_1.Query(filter.toUpperCase());
            // Assert
            expect(query.error).toBeUndefined();
...
(description includes line 1 continued\
 with \ backslash)`,
    ...
    it('should allow spaces between show or hide and a Query option', () => {
        const query1 = new Query_1.Query('show  tree');
    ...
    it('should allow spaces between show or hide and a Task option', () => {
        const query1 = new Query_1.Query('hide  priority');
        expect(query1.taskLayoutOptions.isShown(TaskLayoutOptions_1.TaskLayoutComponent.Priority)).toBe(false);
        expect(query1.error).toBeUndefined();
    });
    it('should parse ambiguous sort by queries correctly', () => {
        expect(new Query_1.Query('sort by status').sorting[0].property).toEqual('status');
        expect(new Query_1.Query('SORT BY STATUS').sorting[0].property).toEqual('status');
        expect(new Query_1.Query('sort by status.name').sorting[0].property).toEqual('status.name');
        expect(new Query_1.Query('SORT BY STATUS.NAME').sorting[0].property).toEqual('status.name');
    });
    it('should parse ambiguous group by queries correctly', () => {
        expect(new Query_1.Query('group by status').grouping[0].property).toEqual('status');
        expect(new Query_1.Query('GROUP BY STATUS').grouping[0].property).toEqual('status');
        expect(new Query_1.Query('group by status.name').grouping[0].property).toEqual('status.name');
        expect(new Query_1.Query('GROUP BY STATUS.NAME').grouping[0].property).toEqual('status.name');
        expect(new Query_1.Query('group by status.type').grouping[0].property).toEqual('status.type');
        expect(new Query_1.Query('GROUP BY STATUS.TYPE').grouping[0].property).toEqual('status.type');
    });
    it('should parse "short mode" and "full mode" correctly', () => {
        // Check 'short' mode is enabled:
        expect(new Query_1.Query('short').queryLayoutOptions.shortMode).toEqual(true);
        expect(new Query_1.Query('short mode').queryLayoutOptions.shortMode).toEqual(true);
        // Check that 'full' reverses short mode:
        expect(new Query_1.Query('short\nfull mode').queryLayoutOptions.shortMode).toEqual(false);
        expect(new Query_1.Query('short mode\nfull').queryLayoutOptions.shortMode).toEqual(false);
    });
    it('should cope with missing end-of-line character in query ending with continuation character - #3137', () => {
        const query = new Query_1.Query('path includes query.md \\');
        expect(query.filters.length).toEqual(1);
    ...
    describe('should include instruction in parsing error messages', () => {
        function getQueryError(source) {
            return new Query_1.Query(source, (0, TasksFileHelpers_1.createTestTasksFile)('Example Path.md')).error;
        }
        it('for invalid regular expression filter', () => {
            const source = 'description regex matches apple sauce';
            expect(getQueryError(source)).toEqual(String.raw `Invalid instruction: 'description regex matches apple sauce'
...
`);
    ...
    describe('parsing placeholders', () => {
        it('should expand placeholder values in filters, but not source', () => {
            ...
            // Act
            const query = new Query_1.Query(rawQuery, tasksFile);
            // Assert
        ...
        it('should report error if placeholders used without query location', () => {
            ...
            // Assert
            expect(query.error).toEqual('The query looks like it contains a placeholder, with "{{" and "}}"\n' +
        ...
        it('should report error if non-existent placeholder used', () => {
            ...
            // Assert
            expect(query.error).toEqual('There was an error expanding one or more placeholders.\n' +
                '\n' +
                'The error message was:\n' +
...
{{error 3}}`;
    ...
    describe('properties in the query file', () => {
        const file = (0, MockDataHelpers_1.getTasksFileFromMockData)('query_using_properties');
        function makeQueryFromPropertyWithValue(propertyName, propertyValue) {
            const source = "{{query.file.property('" + propertyName + "')}}";
            const query = new Query_1.Query(source, file);
...
`;
                ...
                expect(query.explainQuery()).toMatchInlineSnapshot(`
                    ...
                    "
                `);
                function checkNumberOfMatches(expectedNumberOfMatches, path) {
                    const queryResult = query.applyQueryToTasks([new TaskBuilder_1.TaskBuilder().path(path).build()]);
...
describe('Query', () => {
    describe('filtering', () => {
        it('filters paths case insensitive', () => {
            ...
            const tasks = [
                new Task_1.Task({
                    ...
                    taskLocation: TaskLocation_1.TaskLocation.fromUnknownPosition((0, TasksFileHelpers_1.createTestTasksFile)('Ab/C D')),
                    indentation: '',
                    listMarker: '-',
                ...
                new Task_1.Task({
                    status: Status_1.Status.TODO,
                    description: 'description',
                    taskLocation: TaskLocation_1.TaskLocation.fromUnknownPosition((0, TasksFileHelpers_1.createTestTasksFile)('FF/C D')),
                    indentation: '',
                    listMarker: '-',
                    priority: Priority_1.Priority.None,
                    startDate: null,
        ...
        test.concurrent.each([
            ...
            (0, FilterTestHelpers_1.shouldSupportFiltering)(filters.map((filter) => filter.toUpperCase()), allTaskLines, expectedResult);
        });
    });
    describe('filtering with "happens"', () => {
        ...
        test.concurrent.each(HappensCases)('filters via "happens" correctly (%j)', ({ happensFilter, due, scheduled, start, done, taskShouldMatch }) => {
            // Arrange
            const line = [
                '- [ ] this is a task',
                !!start && `🛫 ${start}`,
            ...
            // Act, Assert
            (0, FilterTestHelpers_1.shouldSupportFiltering)([happensFilter], [line], expectedResult);
            (0, FilterTestHelpers_1.shouldSupportFiltering)([happensFilter.toUpperCase()], [line], expectedResult);
        });
    });
    ...
    describe('filtering with code-based custom filters', () => {
        it('should allow a Filter to be added', () => {
            // Arrange
            const filterOrErrorMessage = new DescriptionField_1.DescriptionField().createFilterOrErrorMessage('description includes xxx');
    ...
    describe('query path and metadata', function () {
        it('should provide access to the path of the query', () => {
            ...
            const query = new Query_1.Query('not done', (0, TasksFileHelpers_1.createTestTasksFile)(path));
            expect(query.filePath).toEqual(path);
        });
        it('should give filePath unknown if no path provided to query', () => {
            const query = new Query_1.Query('not done');
    ...
    describe('SearchInfo', () => {
        it('should pass SearchInfo through to filter functions', () => {
            // Arrange
            const same1 = new TaskBuilder_1.TaskBuilder().description('duplicate').build();
            const same2 = new TaskBuilder_1.TaskBuilder().description('duplicate').build();
            const different = new TaskBuilder_1.TaskBuilder().description('different').build();
            const allTasks = [same1, same2, different];
            const moreThanOneTaskHasThisDescription = (task, searchInfo) => {
                return searchInfo.allTasks.filter((t) => t.description === task.description).length > 1;
            };
            const filter = FilterOrErrorMessage_1.FilterOrErrorMessage.fromFilter(new Filter_1.Filter('stuff', moreThanOneTaskHasThisDescription, new Explanation_1.Explanation('explanation of stuff')));
            // Act, Assert
            const searchInfo = SearchInfo_1.SearchInfo.fromAllTasks(allTasks);
        ...
        it('should pass the query path through to filter functions', () => {
            // Arrange
            const queryTasksFile = (0, TasksFileHelpers_1.createTestTasksFile)('this/was/passed/in/correctly.md');
            const query = new Query_1.Query('', queryTasksFile);
            const matchesIfSearchInfoHasCorrectPath = (_task, searchInfo) => {
                return searchInfo.tasksFile === queryTasksFile;
            };
            query.addFilter(new Filter_1.Filter('instruction', matchesIfSearchInfoHasCorrectPath, new Explanation_1.Explanation('explanation')));
            // Act
    ...
    describe('show and hide', () => {
        it('should show "on completion" by default', () => {
            const query = new Query_1.Query('');
            expect(query.taskLayoutOptions.isShown(TaskLayoutOptions_1.TaskLayoutComponent.OnCompletion)).toEqual(true);
        });
        it('should allow to hide "on completion"', () => {
            const query = new Query_1.Query('hide on completion');
            expect(query.taskLayoutOptions.isShown(TaskLayoutOptions_1.TaskLayoutComponent.OnCompletion)).toEqual(false);
        });
    ...
    describe('sorting', () => {
        ...
        it('sort reverse returns -0 for equal tasks', () => {
            ...
            const query = new Query_1.Query('sort by status reverse');
            const sorter = query.sorting[0];
            const searchInfo = SearchInfo_1.SearchInfo.fromAllTasks([todoTask, doneTask]);
    ...
    describe('grouping instructions', () => {
        ...
        it('should parse a supported group command without error', () => {
            ...
            const query = new Query_1.Query(source);
            const queryUpper = new Query_1.Query(source.toUpperCase());
            // Assert
            expect(query.error).toBeUndefined();
        ...
        it('should work with a custom group that uses query information', () => {
            ...
            const tasksFile = (0, TasksFileHelpers_1.createTestTasksFile)('hello.md');
            const query = new Query_1.Query(source, tasksFile);
            const queryUpper = new Query_1.Query(sourceUpper, tasksFile);
            // Act
            const results = query.applyQueryToTasks([new TaskBuilder_1.TaskBuilder().build()]);
            const resultsUpper = queryUpper.applyQueryToTasks([new TaskBuilder_1.TaskBuilder().build()]);
            // Assert
        ...
        it('should log meaningful error for supported group type', () => {
            ...
            const query = new Query_1.Query(source);
            const queryUpperr = new Query_1.Query(sourceUpper);
            // Assert
        ...
        it('should apply limit correctly, after sorting tasks', () => {
            ...
            const sourceUpper = source.toUpperCase();
            const query = new Query_1.Query(source);
            const queryUpper = new Query_1.Query(sourceUpper);
...
wibble`;
            ...
            // Assert
            expect(queryResult.searchErrorMessage).toEqual(`Error: Search failed.
The error message was:
...
`);
            expect(queryResultUpper.searchErrorMessage).toEqual(`Error: Search failed.
The error message was:
    "ReferenceError: WIBBLE is not defined"
Problem statement:
    FILTER BY FUNCTION \\
...
`);
    ...
    describe('line continuations', () => {
        it('should save the source correctly in a Statement object', () => {
            ...
            expect(filter.statement.anyContinuationLinesRemoved).toEqual('(path includes A) OR (path includes {{query.file.path}})');
...
with \ backslash)`;
            const query = new Query_1.Query(source);
            const queryUpperCase = new Query_1.Query(source);
            expect(query.explainQuery()).toMatchInlineSnapshot(`
                "(description includes line 1) OR        \\
                  (description includes line 1 continued\\
                with \\ backslash)
