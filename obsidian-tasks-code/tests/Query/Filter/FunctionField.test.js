...
const yesterdayDate = new TasksDate_1.TasksDate((0, moment_1.default)(yesterday));
const todayDate = new TasksDate_1.TasksDate((0, moment_1.default)(today));
const tomorrowDate = new TasksDate_1.TasksDate((0, moment_1.default)(tomorrow));
const undated = new TasksDate_1.TasksDate(null);
beforeEach(() => {
    jest.useFakeTimers();
...
describe('FunctionField - disabling execution', () => {
    ...
    it('"filter by function" should have meaningful parse-time error', () => {
        ...
        const query = new Query_1.Query(instruction);
...
describe('FunctionField - filtering', () => {
    const functionField = new FunctionField_1.FunctionField();
    ...
    it('filter by function - with valid query of Query property', () => {
        const tasksInSameFileAsQuery = functionField.createFilterOrErrorMessage('filter by function task.file.path === query.file.path');
        expect(tasksInSameFileAsQuery).toBeValid();
        const queryTasksFile = (0, TasksFileHelpers_1.createTestTasksFile)('/a/b/query.md');
        const taskInQueryFile = new TaskBuilder_1.TaskBuilder().path(queryTasksFile.path).build();
        const taskNotInQueryFile = new TaskBuilder_1.TaskBuilder().path('some other path.md').build();
        const searchInfo = new SearchInfo_1.SearchInfo(queryTasksFile, [taskInQueryFile, taskNotInQueryFile]);
    ...
    it('filter by function - should report syntax errors via FilterOrErrorMessage', () => {
        ...
        expect(filter.error).toEqual('Error: Failed parsing expression "task.status.name.toUpperCase())".\nThe error message was:\n    "SyntaxError: Unexpected token \')\'"');
    });
    it('filter by function - should report searching errors via exception', () => {
        ...
        const t = () => {
            ...
            filter.filterFunction(task, SearchInfo_1.SearchInfo.fromAllTasks([task]));
        };
...
describe('FunctionField - sorting', () => {
    describe('basics', () => {
        ...
        it('should parse "sort by function" line', () => {
            // Arrange
            const field = new FunctionField_1.FunctionField();
    ...
    describe('comparing', () => {
        ...
        const BEFORE = -1;
        const AFTER = 1;
        function checkAndCompareSortKeys(valueA, valueB) {
        ...
        it('should refuse to compare unrecognised type', () => {
            const taskA = new TaskBuilder_1.TaskBuilder().description('A').build();
            const taskB = new TaskBuilder_1.TaskBuilder().description('B').build();
            const t = () => {
    ...
    describe('error-handling', () => {
        const field = new FunctionField_1.FunctionField();
        function getQueryErrorMessage(line) {
            const query = new Query_1.Query(line);
            if (query.error) {
                return query.error;
            }
            const task = (0, TestHelpers_1.fromLine)({ line: '- [ ] stuff' });
        ...
        it('should throw if SyntaxError in expression', () => {
            ...
            expect(t).toThrow(Error);
            expect(t).toThrowError(`Error: Failed parsing expression "task.due.formatAsDate(".
The error message was:
    "SyntaxError: Unexpected token '}'"`);
        });
    ...
    describe('example functions', () => {
        ...
        function with_description(description) {
        ...
        it('sort by function - integer expression - smaller numbers come before larger ones', () => {
            // Arrange
            const sorter = new FunctionField_1.FunctionField().createSorterFromLine('sort by function task.description.length');
            // Assert
            expect(sorter).not.toBeNull();
            (0, CustomMatchersForSorting_1.expectTaskComparesEqual)(sorter, with_description('Aaa'), with_description('Aaa'));
            (0, CustomMatchersForSorting_1.expectTaskComparesEqual)(sorter, with_description('AAA'), with_description('ZZZ'));
            // Sorts on string length - shorter first
            (0, CustomMatchersForSorting_1.expectTaskComparesBefore)(sorter, with_description('AAA'), with_description('AAAA'));
            (0, CustomMatchersForSorting_1.expectTaskComparesBefore)(sorter, with_description(''), with_description('B'));
            // Sorts on string length - longer first
            (0, CustomMatchersForSorting_1.expectTaskComparesAfter)(sorter, with_description('xxxx'), with_description('x'));
        });
        it('sort by function - integer expression - reverse', () => {
            // Arrange
            const sorter = new FunctionField_1.FunctionField().createSorterFromLine('sort by function reverse task.description.length');
            // Assert
            expect(sorter).not.toBeNull();
            (0, CustomMatchersForSorting_1.expectTaskComparesEqual)(sorter, with_description('Aaa'), with_description('Aaa'));
            // Sorts on string length - shorter first
            (0, CustomMatchersForSorting_1.expectTaskComparesAfter)(sorter, with_description('AAA'), with_description('AAAA'));
            // Sorts on string length - longer first
            (0, CustomMatchersForSorting_1.expectTaskComparesBefore)(sorter, with_description('xxxx'), with_description('x'));
        });
        it('sort by function - string expression - case-insensitive, number sort', () => {
            // Arrange
            const sorter = new FunctionField_1.FunctionField().createSorterFromLine('sort by function task.originalMarkdown');
            // Assert
            expect(sorter).not.toBeNull();
            (0, CustomMatchersForSorting_1.expectTaskComparesBefore)(sorter, (0, TestHelpers_1.fromLine)({ line: '- [ ] Hello' }), (0, TestHelpers_1.fromLine)({ line: '* [ ] Hello' }));
            (0, CustomMatchersForSorting_1.expectTaskComparesBefore)(sorter, (0, TestHelpers_1.fromLine)({ line: '* [ ] Apple' }), (0, TestHelpers_1.fromLine)({ line: '* [ ] Hello' }));
            (0, CustomMatchersForSorting_1.expectTaskComparesBefore)(sorter, (0, TestHelpers_1.fromLine)({ line: '* [ ] Apple' }), (0, TestHelpers_1.fromLine)({ line: '* [ ] APPLE' })); // case-insensitive
            (0, CustomMatchersForSorting_1.expectTaskComparesAfter)(sorter, (0, TestHelpers_1.fromLine)({ line: '- [ ] 10 Hello' }), (0, TestHelpers_1.fromLine)({ line: '- [ ] 9 Hello' }));
            (0, CustomMatchersForSorting_1.expectTaskComparesAfter)(sorter, (0, TestHelpers_1.fromLine)({ line: '* [ ] Hello' }), (0, TestHelpers_1.fromLine)({ line: '- [ ] Hello' }));
        });
        it('sort by function - boolean expression - true sorts before false', () => {
            // Arrange
            const sorter = new FunctionField_1.FunctionField().createSorterFromLine('sort by function task.isDone');
        ...
        it('sort by function - using query.file.path', () => {
            // Arrange
            const sorter = new FunctionField_1.FunctionField().createSorterFromLine('sort by function task.file.path === query.file.path');
            const todoTask = (0, TestHelpers_1.fromLine)({ line: '- [ ] todo' });
            const doneTask = (0, TestHelpers_1.fromLine)({ line: '- [x] done' });
            // Assert
            expect(sorter).not.toBeNull();
            (0, CustomMatchersForSorting_1.expectTaskComparesEqual)(sorter, todoTask, doneTask);
        });
    });
});
// -----------------------------------------------------------------------------------------------------------------
...
function createGrouper(line) {
    const grouper = new FunctionField_1.FunctionField().createGrouperFromLine(line);
...
describe('FunctionField - grouping - error-handling', () => {
    ...
    it('should give a meaningful error for invalid syntax', () => {
        ...
        (0, CustomMatchersForGrouping_1.toGroupTaskWithPath)(grouper, 'journal/a/b.md', [
    ...
    it('should give a meaningful error for unknown task field', () => {
        ...
        const grouper = createGrouper(line);
        (0, CustomMatchersForGrouping_1.toGroupTaskFromBuilder)(grouper, new TaskBuilder_1.TaskBuilder(), [
...
describe('FunctionField - grouping return types', () => {
    ...
    it.each(expressionsAndResults)("expression: '%s'", (expression, result) => {
        const line = `group by function ${expression}`;
        const grouper = createGrouper(line);
        (0, CustomMatchersForGrouping_1.toGroupTaskFromBuilder)(grouper, new TaskBuilder_1.TaskBuilder(), result);
    });
});
describe('FunctionField - grouping - example functions', () => {
    it('should display booleans in a meaningful way', () => {
        ...
        const grouper = createGrouper(line);
        (0, CustomMatchersForGrouping_1.toGroupTaskWithPath)(grouper, 'journal/a/b.md', ['false']);
    });
    it('using root and path', () => {
        ...
        const grouper = createGrouper(line);
        (0, CustomMatchersForGrouping_1.toGroupTaskWithPath)(grouper, 'journal/a/b.md', ['journal/']);
        (0, CustomMatchersForGrouping_1.toGroupTaskWithPath)(grouper, 'hello/world/from-me.md', ['hello/world/from-me.md']);
        (0, CustomMatchersForGrouping_1.toGroupTaskWithPath)(grouper, 'file-in-root-folder.md', ['file-in-root-folder.md']);
    });
    it('using path stripping folder', () => {
        const line = 'group by function task.path.replace("some/prefix/", "")';
        const grouper = createGrouper(line);
        (0, CustomMatchersForGrouping_1.toGroupTaskWithPath)(grouper, 'a/b/c.md', ['a/b/c.md']);
    });
    it('group by priority', () => {
        const line = 'group by function task.priority';
        const grouper = createGrouper(line);
        (0, CustomMatchersForGrouping_1.toGroupTaskFromBuilder)(grouper, new TaskBuilder_1.TaskBuilder().priority(Priority_1.Priority.Highest), ['0']);
    });
    it('group by status symbol', () => {
        ...
        const grouper = createGrouper(line);
        (0, CustomMatchersForGrouping_1.toGroupTaskFromBuilder)(grouper, new TaskBuilder_1.TaskBuilder().status(Status_1.Status.CANCELLED), ['-']);
        (0, CustomMatchersForGrouping_1.toGroupTaskFromBuilder)(grouper, new TaskBuilder_1.TaskBuilder().status(Status_1.Status.TODO), ['space']);
    });
    it('group by status nextStatusSymbol', () => {
        const line = 'group by function task.status.nextStatusSymbol.replace(" ", "space")';
        const grouper = createGrouper(line);
        (0, CustomMatchersForGrouping_1.toGroupTaskFromBuilder)(grouper, new TaskBuilder_1.TaskBuilder().status(Status_1.Status.IN_PROGRESS), ['x']);
        (0, CustomMatchersForGrouping_1.toGroupTaskFromBuilder)(grouper, new TaskBuilder_1.TaskBuilder().status(Status_1.Status.DONE), ['space']);
    });
    it('group by using number', () => {
        const line = 'group by function task.description.length';
        const grouper = createGrouper(line);
        (0, CustomMatchersForGrouping_1.toGroupTaskFromBuilder)(grouper, new TaskBuilder_1.TaskBuilder().description('#task Hello'), ['11']);
    });
    it('group by using string literal', () => {
        const line = 'group by function reverse "Description length: " + task.description.length';
        const grouper = createGrouper(line);
        (0, CustomMatchersForGrouping_1.toGroupTaskFromBuilder)(grouper, new TaskBuilder_1.TaskBuilder().description('#task Hello'), ['Description length: 11']);
    });
    it('group by a selected tag', () => {
        const line = "group by function task.tags.find((tag) => tag.includes('#context/'))";
        const grouper = createGrouper(line);
        (0, CustomMatchersForGrouping_1.toGroupTaskFromBuilder)(grouper, new TaskBuilder_1.TaskBuilder().tags(['#context/pc_home', '#topic/sys_admin']), [
            '#context/pc_home',
        ]);
    });
    it('group by a query property', () => {
        ...
        const grouper = createGrouper(line);
        const task = new TaskBuilder_1.TaskBuilder().build();
        (0, CustomMatchersForGrouping_1.toGroupTaskUsingSearchInfo)(grouper, task, new SearchInfo_1.SearchInfo((0, TasksFileHelpers_1.createTestTasksFile)('queries/query file.md'), [task]), ['query file.md']);
    });
});
//# sourceMappingURL=FunctionField.test.js.map