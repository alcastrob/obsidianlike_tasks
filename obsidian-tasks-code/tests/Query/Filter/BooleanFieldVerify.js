...
exports.verifyBooleanExpressionPreprocessing = verifyBooleanExpressionPreprocessing;
exports.verifyBooleanExpressionExplanation = verifyBooleanExpressionExplanation;
...
`;
function verifyBooleanExpressionPreprocessing(fn) {
    (0, JestApprovals_1.verifyAll)('Results of preprocessing boolean expressions', exports.inputs.split('\n'), (input) => {
        if (input.trim() === '') {
            return '';
        }
        let result = '';
        try {
            const delimiters = BooleanDelimiters_1.BooleanDelimiters.fromInstructionLine(input);
            result = JSON.stringify(fn(input, delimiters), null, 4);
        }
        catch (e) {
            result = (0, ExceptionTools_1.errorMessageForException)('Parsing expression', e);
        }
        return `
Input:
'${input}'
...
function verifyBooleanExpressionExplanation() {
    (0, JestApprovals_1.verifyAll)('Results of parsing and explaining boolean expressions', exports.inputs.split('\n'), (input) => {
        if (input.trim() === '') {
            return '';
        }
        let result = '';
        const filterOrErrorMessage = new BooleanField_1.BooleanField().createFilterOrErrorMessage(input);
...
//# sourceMappingURL=BooleanFieldVerify.js.map