...
const BooleanPreprocessor_1 = require("../../../src/Query/Filter/BooleanPreprocessor");
const TasksFileHelpers_1 = require("../../TestingTools/TasksFileHelpers");
const BooleanFieldVerify_1 = require("./BooleanFieldVerify");
window.moment = moment_1.default;
function testWithDescription(filter, description, expected) {
    const builder = new TaskBuilder_1.TaskBuilder();
    (0, FilterTestHelpers_1.testFilter)(filter, builder.description(description), expected);
}
function createValidFilter(instruction) {
    // First make sure that BooleanField recognises the instruction as valid
    expect(new BooleanField_1.BooleanField().canCreateFilterForLine(instruction)).toEqual(true);
    return new BooleanField_1.BooleanField().createFilterOrErrorMessage(instruction);
}
describe('boolean query - filter', () => {
    ...
    describe('basic operators', () => {
        ...
        it('input components dirty with whitespaces', () => {
            const filter = createValidFilter(' (description includes #context/location1) OR (description includes #context/location2 ) OR (  description includes #context/location3 ) OR   (  description includes #context/location4 )');
    ...
    describe('delimiters', () => {
        function explanationOrError(filter) {
            if (filter.filter) {
        ...
        describe('( and )', () => {
            ...
            it('should allow ( and ) as delimiters around 2 filters', () => {
                const filter = createValidFilter('(description includes #context/location1) OR (description includes #context/location2)');
                expect(explanationOrError(filter)).toMatchInlineSnapshot(`
        ...
        describe('Mixed delimiters', () => {
            it('should not allow a mixture of () and "" delimiters any more - breaking change in 7.0.0', () => {
                const filter = new BooleanField_1.BooleanField().createFilterOrErrorMessage('(not done) AND "is recurring"');
    ...
    describe('error cases - to show error messages', () => {
        ...
        ])('should report expected error message: on "%s" - expected "%s"', (instruction, expectedError) => {
            const filter = new BooleanField_1.BooleanField().createFilterOrErrorMessage(instruction);
    ...
    describe('former error cases, that now work', () => {
        it.each([
            ...
            ['(path includes a) XOR(path includes b)'],
            ['NOT(path includes b)'],
        ])('should report expected error message: on "%s" - expected "%s"', (instruction) => {
            const filter = new BooleanField_1.BooleanField().createFilterOrErrorMessage(instruction);
...
describe('boolean query - explain', () => {
    ...
    function explainFilters(indentationLevel, source) {
        const indentation = ' '.repeat(indentationLevel);
        const tasksFile = (0, TasksFileHelpers_1.createTestTasksFile)('some/sample/note.md');
        const query1 = new Query_1.Query(source, tasksFile);
        return new Explainer_1.Explainer(indentation).explainFilters(query1);
    }
...
describe('boolean query - exhaustive tests', () => {
    ...
    it('preprocess - split line', () => {
        (0, BooleanFieldVerify_1.verifyBooleanExpressionPreprocessing)(BooleanPreprocessor_1.BooleanPreprocessor.splitLine);
    });
    it('preprocess - rewrite', () => {
        (0, BooleanFieldVerify_1.verifyBooleanExpressionPreprocessing)(BooleanPreprocessor_1.BooleanPreprocessor.preprocessExpression);
    });
    it('explain', () => {
        (0, BooleanFieldVerify_1.verifyBooleanExpressionExplanation)();
    });
});
