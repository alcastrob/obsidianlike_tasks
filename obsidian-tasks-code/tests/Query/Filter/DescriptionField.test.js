...
function testDescriptionFilter(filter, line, expected) {
...
describe('description should strip signifiers, some duplicate spaces and trailing spaces', () => {
    const field = new DescriptionField_1.DescriptionField();
    ...
    it('with non-tag as global filter - all tags included', () => {
        // Arrange
        GlobalFilter_1.GlobalFilter.getInstance().set('global-filter');
        const task = (0, TestHelpers_1.fromLine)({
...
describe('description', () => {
    it('ignores the global filter when filtering', () => {
        // Arrange
        GlobalFilter_1.GlobalFilter.getInstance().set('#task');
        const filter = new DescriptionField_1.DescriptionField().createFilterOrErrorMessage('description includes task');
        // Act, Assert
        testDescriptionFilter(filter, '- [ ] #task this does not include the word; only in the global filter', false);
    ...
    it('works with regex', () => {
        // Arrange
        const filter = new DescriptionField_1.DescriptionField().createFilterOrErrorMessage('description regex matches /^task/');
        // Assert
    ...
    it('works negating regexes', () => {
        // Arrange
        const filter = new DescriptionField_1.DescriptionField().createFilterOrErrorMessage('description regex does not match /^task/');
        // Assert
...
describe('search description for time stamps', () => {
    it('should find a time stamp in the description - simple version', () => {
        // Arrange
        const filter = new DescriptionField_1.DescriptionField().createFilterOrErrorMessage(String.raw `description regex matches /\d\d:\d\d/`);
        // Assert
    ...
    it('should find a time stamp in the description - more precise version', () => {
        // Arrange
        const filter = new DescriptionField_1.DescriptionField().createFilterOrErrorMessage('description regex matches /[012][0-9]:[0-5][0-9]/');
        // Assert
        expect(filter).toMatchTaskFromLine('- [ ] Do me at 23:59');
        expect(filter).toMatchTaskFromLine('- [ ] Do me at 00:01');
...
describe('search description for short tags, excluding sub-tags', () => {
    ...
    it('should search for a short tag anywhere in the line except the end', () => {
        ...
        const filter = new DescriptionField_1.DescriptionField().createFilterOrErrorMessage(String.raw `description regex matches /#t\s/i`);
        // Assert
    ...
    it('should search for a short tag at the end of the task', () => {
        ...
        const filter = new DescriptionField_1.DescriptionField().createFilterOrErrorMessage('description regex matches /#t$/i');
        // Assert
    ...
    it('should search for a short tag anywhere', () => {
        // Arrange
        const filter = new BooleanField_1.BooleanField().createFilterOrErrorMessage(String.raw `(description regex matches /#t\s/i) OR (description regex matches /#t$/i)`);
        // Assert
        expect(filter).toMatchTaskFromLine('- [ ] #t Do stuff');
        expect(filter).toMatchTaskFromLine('- [ ] Do #t stuff');
        expect(filter).toMatchTaskFromLine('- [ ] Do stuff #t');
...
describe('search description for sub-tags', () => {
    it('should search for a short tag anywhere', () => {
        // Arrange
        const filter = new DescriptionField_1.DescriptionField().createFilterOrErrorMessage(String.raw `description regex matches /#tag\/subtag[0-9]\/subsubtag[0-9]/i`);
        // Assert
...
describe('search description for Alternation (OR)', () => {
    it('should search for one of several spellings of waiting', () => {
        ...
        // Arrange
        const filter = new DescriptionField_1.DescriptionField().createFilterOrErrorMessage(String.raw `description regex matches /waiting|waits|wartet/i`);
        // Assert
...
describe('sorting by description', () => {
    ...
    it('sort by description', () => {
        // Arrange
        const sorter = new DescriptionField_1.DescriptionField().createNormalSorter();
        // Assert
        (0, CustomMatchersForSorting_1.expectTaskComparesEqual)(sorter, with_description('Aaa'), with_description('Aaa'));
        (0, CustomMatchersForSorting_1.expectTaskComparesBefore)(sorter, with_description('AAA'), with_description('ZZZ'));
        // Ignores case if strings differ
        (0, CustomMatchersForSorting_1.expectTaskComparesBefore)(sorter, with_description('AAA'), with_description('bbb'));
        (0, CustomMatchersForSorting_1.expectTaskComparesBefore)(sorter, with_description('aaa'), with_description('BBB'));
        (0, CustomMatchersForSorting_1.expectTaskComparesBefore)(sorter, with_description('aaa'), with_description('AAA'));
        // Sorts text with numbers in correctly
        (0, CustomMatchersForSorting_1.expectTaskComparesBefore)(sorter, with_description('9 x'), with_description('11 x'));
        (0, CustomMatchersForSorting_1.expectTaskComparesBefore)(sorter, with_description('x 9'), with_description('x 11'));
    });
    it('sort by description reverse', () => {
        // Single example just to prove reverse works.
        // (There's no need to repeat all the examples above)
        const sorter = new DescriptionField_1.DescriptionField().createReverseSorter();
        (0, CustomMatchersForSorting_1.expectTaskComparesAfter)(sorter, with_description('AAA'), with_description('ZZZ'));
    });
    describe('show how markdown in descriptions gets cleaned', () => {
        ...
        it('characters that are not stripped out', () => {
            ...
            (0, CustomMatchersForSorting_1.expectTaskComparesBefore)(sorter, new TaskBuilder_1.TaskBuilder()
                .description('*ZZZ Initial lone asterisk is not stripped, so these two tasks sort unequal')
                .build(), new TaskBuilder_1.TaskBuilder()
                .description('ZZZ Initial lone asterisk is not stripped, so these two tasks sort unequal')
                .build());
        });
        ...
        it.each([
            ...
            '[un-format]',
            '==un-format==',
        ])('simple description "%s" is cleaned to "un-format"', (originalDescription) => {
            expect(DescriptionField_1.DescriptionField.cleanDescription(originalDescription)).toStrictEqual('un-format');
        });
        ...
        ])('description "%s" is cleaned to "%s"', (originalDescription, cleanedDescription) => {
            expect(DescriptionField_1.DescriptionField.cleanDescription(originalDescription)).toStrictEqual(cleanedDescription);
            (0, CustomMatchersForSorting_1.expectTaskComparesEqual)(sorter, new TaskBuilder_1.TaskBuilder().description(originalDescription).build(), new TaskBuilder_1.TaskBuilder().description(cleanedDescription).build());
        });
    });
    ...
    ])('description "%s" is unchanged when cleaned"', (originalDescription) => {
        expect(DescriptionField_1.DescriptionField.cleanDescription(originalDescription)).toStrictEqual(originalDescription);
    });
    it('sorts correctly by the link name and not the markdown', () => {
        const one = (0, TestHelpers_1.fromLine)({
            line: '- [ ] *ZZZ An early task that starts with an A; actually not italic since only one asterisk',
        });
        const two = (0, TestHelpers_1.fromLine)({
            line: '- [ ] [[Better be second]] with bla bla behind it',
        });
        const three = (0, TestHelpers_1.fromLine)({
            line: '- [ ] [[Another|Third it should be]] and not [last|ZZZ]',
        });
        const four = (0, TestHelpers_1.fromLine)({
            line: '- [ ] *Very italic text*',
        });
        const five = (0, TestHelpers_1.fromLine)({
            line: '- [ ] [@Zebra|Zebra] should be last for Zebra',
        });
        const expectedOrder = [one, two, three, four, five];
        expect((0, SortingTestHelpers_1.sortBy)([new DescriptionField_1.DescriptionField().createNormalSorter()], [two, one, five, four, three])).toEqual(expectedOrder);
    });
});
//# sourceMappingURL=DescriptionField.test.js.map