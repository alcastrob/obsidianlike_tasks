...
describe('Global Filter tests', () => {
    it('getInstance() should return the same object', () => {
        const globalFilter1 = GlobalFilter_1.GlobalFilter.getInstance();
        const globalFilter2 = GlobalFilter_1.GlobalFilter.getInstance();
    ...
    it('Should allow independent values for independent instances', () => {
        // Arrange
        const globalFilter1 = new GlobalFilter_1.GlobalFilter();
        const globalFilter2 = new GlobalFilter_1.GlobalFilter();
        // Act
        globalFilter1.set('1');
        globalFilter2.set('2');
        // Assert
...
describe('Global Filter tests with Remove Global Filter Setting', () => {
    it('Should remove Global Filter from a string when Setting is set to false', () => {
        ...
        globalFilter.setRemoveGlobalFilter(false);
        // Assert
        expect(globalFilter.removeAsWordFromDependingOnSettings('This is absolutely todo')).toEqual('This is absolutely todo');
    });
    it('Should remove Global Filter from a string when Setting is set to true', () => {
        ...
        globalFilter.setRemoveGlobalFilter(true);
        // Assert
        expect(globalFilter.removeAsWordFromDependingOnSettings('This is absolutely todo')).toEqual('This is absolutely');
    });
    it.each([
    ...
    ])('should not remove Global Filter (%s) if it is in a substring for a task "- [ ] %s"', async (globalFilterValue, description) => {
        const globalFilter = new GlobalFilter_1.GlobalFilter();
        globalFilter.setRemoveGlobalFilter(true);
...
describe('check removal of the global filter', () => {
    ...
    ])('should parse "$inputDescription" and extract "$expectedDescription"', ({ globalFilterValue, inputDescription, expectedDescription }) => {
        // Arrange
        const globalFilter = new GlobalFilter_1.GlobalFilter();
        globalFilter.set(globalFilterValue);
        // Assert
...
//# sourceMappingURL=GlobalFilter.test.js.map